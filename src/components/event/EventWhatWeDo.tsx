        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4 md:gap-8">
          {eventServices.map((service, i) => {
            ...
            return (
              ...
              >
                {/* Icon Stamp Frame */}
                <div className="flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-lg border border-ink/10 bg-[#f8f6f0] text-x-red transition-all duration-300 group-hover:border-x-red group-hover:bg-x-red/5 group-hover:scale-105">
                  <EventIconFrame size="lg" className="[&_svg]:size-9 sm:[&_svg]:size-10 text-x-red">
                    <Icon />
                  </EventIconFrame>
                </div>

                {/* FormX Red Service Title */}
                <h3 className="mt-5 font-display text-[16.5px] sm:text-[18px] font-black uppercase leading-tight tracking-tight text-x-red">
                  {service.title}
                </h3>

                {/* Direct Description Paragraph */}
                <p className="mt-2.5 font-body text-[14.5px] sm:text-[15.5px] font-medium leading-relaxed text-ink/80">
                  {service.description}
                </p>
